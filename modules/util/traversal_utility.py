...
def collection_traversal(current_element, global_manager, **kwargs):
    ...
    if hasattr(current_element, 'members'):
        for member_element in current_element.members:
            if pretraversal_action(member_element, global_manager): #pretraversal returns False to skip lower traversals if they would have no effect
                if pretraversal_action != alternative_action:
                    if condition(member_element):
                        collection_traversal(member_element, global_manager, pretraversal_action=pretraversal_action, alternative_action=alternative_action, condition=condition, posttraversal_action=posttraversal_action)
                    else: #if condition false, do alternative action for every element below without checking condition
                        collection_traversal(member_element, global_manager, pretraversal_action=alternative_action, alternative_action=alternative_action, condition=condition)
