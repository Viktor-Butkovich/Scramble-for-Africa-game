...
def draw_interface_elements(interface_elements):
    ...
    for current_interface_element in interface_elements:
        collection_traversal(current_interface_element, pretraversal_action=set_showing, alternative_action=set_not_showing, condition=check_showing,
...
def collection_traversal(current_element, **kwargs):
    ...
    if hasattr(current_element, 'members'):
        for member_element in current_element.members:
            if pretraversal_action(member_element): #pretraversal returns False to skip lower traversals if they would have no effect
                if pretraversal_action != alternative_action:
                    if condition(member_element):
                        collection_traversal(member_element, pretraversal_action=pretraversal_action, alternative_action=alternative_action, condition=condition, posttraversal_action=posttraversal_action)
                    else: #if condition false, do alternative action for every element below without checking condition
                        collection_traversal(member_element, pretraversal_action=alternative_action, alternative_action=alternative_action, condition=condition)
...
def set_showing(current_element):
    ...
    return(old_showing or current_element.showing) #if wasn't showing and still not showing, lower collection elements don't need to be updated - can skip traversal
