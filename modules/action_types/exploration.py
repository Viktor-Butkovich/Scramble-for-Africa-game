...
class exploration(action.action):
        ...
        if subject == 'confirmation':
            ...
            if self.future_cell.resource != 'none':
                if self.future_cell.resource == 'natives':
                    ...
                    text += 'The expedition has discovered a ' + self.future_cell.terrain.upper() + ' tile with a ' + self.future_cell.resource.upper() + ' resource (currently worth ' + str(constants.item_prices[self.future_cell.resource]) + ' money each). /n /n'
    ...
    def generate_attached_interface_elements(self, subject):
        ...
        if subject in ['success', 'critical_success']:
            return_list.append(action_utility.generate_free_image_input_dict(
                action_utility.generate_tile_image_id_list(self.future_cell, force_visibility=True),
                250,
                override_input_dict={
                    'member_config': {
