...
class loan_search(action.campaign):
    ...
    def update_tooltip(self):
        ...
        Output:
            None
        '''
        return(['Attempts to find a 100 money loan offer with a favorable interest rate for ' + str(self.get_price()) + ' money',
                ...
                'Each ' + self.name + ' attempted doubles the cost of other loan searches in the same turn'
        ])
    ...
    def generate_notification_text(self, subject):
        ...
        if subject == 'confirmation':
            ...
            text += 'Loan offer: /n /n'
            text += 'The company will be provided an immediate sum of ' + str(self.current_proposed_loan['principal']) + ' money. /n /n'
            text += 'In return, the company will be obligated to pay back ' + str(self.current_proposed_loan['interest']) + ' money per turn for 10 turns, for a total of ' + str(self.current_proposed_loan['total_paid']) + ' money. /n /n'
        ...
        if super().start(unit):
            constants.notification_manager.display_notification({
                ...
                'choices': [
                    ...
                    'tooltip': ['Starts a search for a low-interest loan offer'],
                    'message': 'Find loan'
                    },
                    {
    ...
    def middle(self):
        ...
        while result < 5:
            ...
            for i in range(num_dice):
                result = max(self.current_unit.controlling_minister.no_corruption_roll(6, self.action_type), result)
            num_attempts += 1
            self.current_proposed_loan['interest'] += 1

        if self.current_unit.controlling_minister.check_corruption(): #no money stolen immediately, but may pay bribe to prosecutor if caught giving false interest amount
        ...
        self.current_proposed_loan['total_paid'] = self.current_proposed_loan['interest'] * 10 #12 interest -> 120 paid
        self.current_proposed_loan['interest_percent'] = (self.current_proposed_loan['interest'] - 10) * 10 #12 interest -> 20%

        if (not self.current_unit.veteran) and result == 6 and num_attempts == 1: #promote if rolled 6 on first attempt and not yet veteran
    ...
    def accept_loan(self):
        ...
        if self.current_proposed_loan['corrupt']:
            self.current_unit.controlling_minister.steal_money(20, 'loan_interest', allow_prosecutor_detection=False)
