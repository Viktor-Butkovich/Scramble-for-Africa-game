...
class rumor_search(action.action):
    ...
    def generate_notification_text(self, subject):
        ...
        if subject == 'confirmation':
            text = 'Are you sure you want to attempt to search for artifact rumors? If successful, the coordinates of a possible location for the ' + status.current_lore_mission.name + ' will be revealed. /n /n'
            ...
            if status.current_lore_mission.get_num_revealed_possible_artifact_locations() == len(status.current_lore_mission.possible_artifact_locations):
                self.current_unit.images[0].current_cell.get_building('village').found_rumors = True
    ...
    def on_click(self, unit):
        ...
        if super().on_click(unit):
            ...
            if not status.current_lore_mission:
                ...
                text_utility.print_to_screen('This village has no population, so no rumors can be found.')
    ...
    def complete(self):
        ...
        location = status.current_lore_mission.get_random_unrevealed_possible_artifact_location()
