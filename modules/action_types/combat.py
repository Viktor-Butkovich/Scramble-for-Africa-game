...
class combat(action.action):
    ...
    def generate_notification_text(self, subject):
        ...
        if subject == 'confirmation':
            ...
            if self.defending:
                text += utility.capitalize(self.opponent.name) + ' ' + utility.conjugate('be', self.opponent.number) + ' attacking your ' + self.current_unit.name + ' at (' + str(self.current_unit.x) + ', ' + str(self.current_unit.y) + ').'
            else:
                text += 'Your ' + self.current_unit.name + ' ' + utility.conjugate('be', self.current_unit.number) + ' attacking the ' + self.opponent.name + ' at (' + str(self.current_unit.x) + ', ' + str(self.current_unit.y) + ').'

        elif subject == 'modifier_breakdown':
            text += 'The ' + self.current_unit.name + ' ' + utility.conjugate('attempt', self.current_unit.number) +' to defeat the ' + self.opponent.name + '. /n /n'

            if self.current_unit.veteran:
                text += 'The ' + self.current_unit.officer.name + ' can roll twice and pick the higher result. /n'
            ...
            if self.current_unit.is_battalion:
                ...
                text += 'As a non-military unit, your ' + self.current_unit.name + ' will receive a -1 penalty after their roll. /n'
                
            if self.current_unit.disorganized:
                text += 'The ' + self.name + ' ' + utility.conjugate('be', self.current_unit.number) + ' disorganized and will receive a -1 penalty after their roll. /n'
            elif self.opponent.disorganized:
                if self.opponent.npmob_type == 'beast':
                    text += 'The ' + self.opponent.name + ' ' + utility.conjugate('be', self.opponent.number) + ' injured and will receive a -1 after its roll. /n'
                else:
                    text += 'The ' + self.opponent.name + ' ' + utility.conjugate('be', self.opponent.number) + ' disorganized and will receive a -1 after their roll. /n'
            ...
            if self.opponent_roll_modifier > 0:
                text += ' + ' + str(self.opponent_roll_modifier) + ' = ' + str(self.opponent_roll_result + self.opponent_roll_modifier)
            elif self.opponent_roll_modifier < 0:
                text += ' - ' + str(self.opponent_roll_modifier * -1) + ' = ' + str(self.opponent_roll_result + self.opponent_roll_modifier)
            text += ' /n'
            ...
            if self.defending:
                if self.opponent.npmob_type == 'beast':
                    ...
                    if self.current_unit.number == 1:
                        text += 'The ' + self.opponent.name + ' slaughtered your ' + self.current_unit.name + '. /n /n'
                    else:
                        text += 'The ' + self.opponent.name + ' slaughtered most of your ' + self.current_unit.name + ' and the survivors deserted, promising to never return. /n /n'
                    ...
                    if self.current_unit.number == 1:
                        text += 'The ' + self.opponent.name + ' decisively defeated your ' + self.current_unit.name + ', who was either slain or captured. /n /n'
                    else:
                        text += 'The ' + self.opponent.name + ' decisively defeated your ' + self.current_unit.name + ', who have all been slain or captured. /n /n'
                ...
                if self.opponent.npmob_type == 'beast':
                    text += 'Your ' + self.current_unit.name + ' were slowly picked off as they tracked the ' + self.opponent.name + ' to its lair. The survivors eventually fled in terror and will be vulnerable to counterattack. /n /n'
                else:
                    text += 'The ' + self.opponent.name + ' decisively routed your ' + self.current_unit.name + ', who ' + utility.conjugate('be', self.opponent.number) +'  scattered and will be vulnerable to counterattack. /n /n'

        elif subject == 'failure': #draw
            if self.defending:
                if self.opponent.last_move_direction[0] > 0: #if enemy attacked by going east
                    retreat_direction = 'west'
                elif self.opponent.last_move_direction[0] < 0: #if enemy attacked by going west
                    retreat_direction = 'east'
                elif self.opponent.last_move_direction[1] > 0: #if enemy attacked by going north
                    retreat_direction = 'south'
                elif self.opponent.last_move_direction[1] < 0: #if enemy attacked by going south
                    retreat_direction = 'north'
                if self.opponent.npmob_type == 'beast':
                    text += 'Your ' + self.current_unit.name + ' managed to scare off the attacking ' + self.opponent.name + ', which was seen running to the ' + retreat_direction + '. /n /n'           
                else:
                    text += 'Your ' + self.current_unit.name + ' managed to repel the attacking ' + self.opponent.name + ', who ' + utility.conjugate('be', self.opponent.number, 'preterite') + ' seen withdrawing to the ' + retreat_direction + '. /n /n'
            else:
                if self.opponent.npmob_type == 'beast':
                    text += 'Your ' + self.current_unit.name + ' failed to track the ' + self.opponent.name + ' to its lair and ' + utility.conjugate('be', self.current_unit.number, 'preterite') + ' forced to withdraw. /n /n'
                else:
                    text += 'Your ' + self.current_unit.name + ' failed to push back the defending ' + self.opponent.name + ' and ' + utility.conjugate('be', self.current_unit.number, 'preterite') + ' forced to withdraw. /n /n'
            ...
            if self.defending:
                if self.opponent.last_move_direction[0] > 0: #if enemy attacked by going east
                    ...
                    retreat_direction = 'north'
                if self.opponent.npmob_type == 'beast':
                    text += 'Your ' + self.current_unit.name + ' injured and scared off the attacking ' + self.opponent.name + ', which was seen running to the ' + retreat_direction + ' and will be vulnerable as it heals. /n /n'           
                else:
                    text += 'Your ' + self.current_unit.name + ' decisively routed the attacking ' + self.opponent.name + ', who ' + utility.conjugate('be', self.opponent.number, 'preterite') + ' seen scattering to the ' + retreat_direction
            ...
            text += 'This ' + self.current_unit.name + '\'s ' + self.current_unit.officer.name + ' is now a veteran. /n /n'
        return(text) 
    ...
    def on_click(self, unit, on_click_info_dict=None):
        ...
        if on_click_info_dict.get('attack_confirmed', False):
            ...
            future_cell = unit.grid.find_cell(self.x_change + unit.x, self.y_change + unit.y)
            opponent = 'none'
    ...
    def pre_start(self, unit):
        ...
        self.current_max_crit_fail = 0
        if (unit.is_safari and self.opponent.npmob_type == 'beast') or (unit.is_battalion and self.opponent.npmob_type != 'beast'):
    ...
    def middle(self, combat_info_dict=None):
        ...
        if self.defending: #if being attacked on main grid, move minimap there to show location
    ...
    def complete(self):
        ...
        if not self.defending:
            ...
            if combat_cell.terrain == 'water' and combat_cell.y > 0 and not self.current_unit.can_swim_river: #if attacked river and can't swim, become disorganized after combat
                self.current_unit.set_disorganized(True)
