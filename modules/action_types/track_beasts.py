...
class track_beasts(action.action):
    ...
    def update_tooltip(self):
        ...
        Output:
            None
        '''
        return(['Attempts to reveal beasts in this tile and adjacent tiles',
    ...
    def can_show(self):
        ...
        return(super().can_show() and 
               ...
               status.displayed_mob.group_type == 'safari'
        )
            ...
            if result >= 4:
                for current_beast in status.beast_list:
                    if current_beast.hidden:
                        if utility.find_grid_distance(unit, current_beast) <= 1: #if different by 1 in x or y or at same coordinates
                            beast_cell = unit.grids[0].find_cell(current_beast.x, current_beast.y)
