...
class artifact_search(action.action):
    ...
    def update_tooltip(self):
        ...
        return(['Attempts to search for ' + description + ' at a rumored location for ' + str(constants.action_prices[self.action_type]) + ' money',
    ...
    def generate_notification_text(self, subject):
        ...
        if subject == 'confirmation':
            text = 'Are you sure you want to attempt to search for the ' + status.current_lore_mission.name + '? '
            text += 'If successful, finds the ' + status.current_lore_mission.name + ' and completes the mission, or verifies that it is located elsewhere. /n /n'
            ...
            location = status.current_lore_mission.get_possible_artifact_location(self.current_unit.x, self.current_unit.y)
    ...
    def on_click(self, unit):
        ...
        if super().on_click(unit):
            ...
            elif not status.current_lore_mission.has_revealed_possible_artifact_location(unit.x, unit.y):
    ...
    def complete(self):
        ...
        if self.roll_result >= self.current_min_success:
            ...
            if location == status.current_lore_mission.artifact_location:
                ...
                text = 'The ' + status.current_country.government_type_adjective.capitalize() + ' Geographical Society awarded ' + str(self.prize_money) + ' money for finding the ' + status.current_lore_mission.name + '. /n /n'
                text += 'Additionally, public opinion has increased by ' + str(self.public_opinion_increase) + '. /n /n'
                self.lore_type = status.current_lore_mission.lore_type
                if not self.lore_type in constants.completed_lore_mission_types:
                    text += 'Completing a mission in the ' + self.lore_type + ' category has given your company a permanent ' + constants.lore_types_effect_descriptions_dict[self.lore_type] + '. /n /n'
                constants.notification_manager.display_notification({
                    'message': text,
                    'on_reveal': self.complete_lore_mission
                })
            else:
