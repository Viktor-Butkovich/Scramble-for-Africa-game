...
class canoe_purchase(action.action):
    ...
    def button_setup(self, initial_input_dict):
        ...
        initial_input_dict['image_id'] = ['buttons/default_button.png', 'misc/green_circle.png', 'items/canoes.png']
    ...
    def update_tooltip(self):
        ...
        Output:
            None
        '''
        return(['Attempts to purchase canoes from this village for ' + str(constants.action_prices[self.action_type]) + ' money',
    ...
    def generate_notification_text(self, subject):
        ...
        if subject == 'confirmation':
            ...
            text += 'The ' + self.name + ' will cost ' + str(constants.action_prices[self.action_type]) + ' money. /n /n'
            ...
            text += 'The ' + status.displayed_mob.group_type + ' tries to purchase canoes. /n /n'
        elif subject == 'success':
            text += 'The villagers were successfully persuaded to sell canoes. /n /n'
        elif subject == 'failure':
            text += 'The ' + status.displayed_mob.group_type + ' failed to make any fruitful transactions. /n /n'
        elif subject == 'critical_failure':
            text += self.generate_notification_text('failure')
            text += 'Angered by the ' + status.displayed_mob.group_type + '\'s offer, the natives attack the expedition. /n /n'
        elif subject == 'critical_success':
            text += self.generate_notification_text('success')
            text += 'The ' + status.displayed_mob.officer.name + ' is now a veteran and will be more successful in future ventures. /n /n'
        return(text)
    ...
    def generate_current_roll_modifier(self):
        ...
        self.aggressiveness_modifier = status.displayed_mob.images[0].current_cell.get_building('village').get_aggressiveness_modifier()
    ...
    def can_show(self):
        ...
        return(super().can_show() and
               ...
               (status.displayed_mob.group_type == 'expedition' or status.displayed_mob.group_type == 'safari')
        )
    ...
    def on_click(self, unit):
        ...
        if super().on_click(unit):
            ...
            if village == 'none':
                text_utility.print_to_screen('Purchasing canoes is only possible in a village.')
            elif village.population <= 0:
                text_utility.print_to_screen('This village has no population, so no canoes can be purchased.')
    ...
    def complete(self):
        ...
        if self.roll_result >= self.current_min_success:
            text = 'The villagers offer to outfit the ' + self.current_unit.group_type + ' with canoes. /n /n'
