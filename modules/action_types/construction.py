    ...
    def update_tooltip(self):
        ...
        if (
            ...
            if not self.building_type in ["train", "steamboat"]:
                message.append(f"{utility.generate_capitalized_article(self.building_name)}{self.building_name} {utility.conjugate('cost', 1, self.building_name)} {base_cost} money by default, which is multiplied by {constants.terrain_build_cost_multiplier_dict[terrain]} when built in {terrain} terrain")
        return message
