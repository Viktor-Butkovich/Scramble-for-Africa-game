...
import random
from . import action
from ..util import action_utility, text_utility, trial_utility, utility, minister_utility, game_transitions, scaling
...
class trial(action.campaign):
    ...
    def button_setup(self, initial_input_dict):
        ...
        '''
        button_width = 150
        initial_input_dict['coordinates'] = scaling.scale_coordinates((constants.default_display_width / 2) - (button_width / 2), constants.default_display_height / 2 - (button_width / 2))
    ...
    def update_tooltip(self):
        ...
        Output:
            None
        '''
        return(['Tries the defending minister in an attempt to remove him from office and imprison him for corruption',
    ...
    def generate_notification_text(self, subject):
        ...
        if subject == 'confirmation':
            text += 'Are you sure you want to start a trial against ' + status.displayed_defense.name + '? You have ' + str(status.displayed_defense.corruption_evidence) + ' pieces of evidence to use. /n /n'
            ...
            text += 'Along with any money paid for bribery or fabricated evidence, a trial fee of ' + str(self.get_price()) + ' money is also required. /n /n'
        elif subject == 'initial':
            new_clothing_types = ['jewelry', 'robes', 'wig']
            if not (self.current_trial['defense_bribed_judge'] or flags.prosecution_bribed_judge):
            ...
            elif self.current_trial['defense_bribed_judge'] and not flags.prosecution_bribed_judge:
            ...
            elif flags.prosecution_bribed_judge and not self.current_trial['defense_bribed_judge']:
                if self.current_trial['judge_bias'] == 1 or self.current_trial['prosecutor_corrupt']: #judge never received the money
                ...
                if self.current_trial['judge_bias'] <= 2:
                    ...
                    self.current_trial['judge_modifier'] = 1
            text += ' /n /n'
            self.current_trial['effective_evidence'] += self.current_trial['judge_modifier']
        
            if self.current_trial['num_lawyers'] == 0:
                text += 'As the defense decided not to hire any additional lawyers, each piece of evidence remains usable, allowing ' + str(self.current_trial['effective_evidence']) + ' evidence rolls to attempt to win the trial. /n /n'
            else:
                text += 'The defense hired ' + str(self.current_trial['num_lawyers']) + ' additional lawyer' + utility.generate_plural(self.current_trial['num_lawyers']) + ', cancelling out ' + str(self.current_trial['num_lawyers']) + ' piece'
                text += utility.generate_plural(self.current_trial['num_lawyers']) + ' of evidence. This leaves ' + str(self.current_trial['effective_evidence']) + ' evidence roll'
                text += utility.generate_plural(self.current_trial['effective_evidence']) + ' to attempt to win the trial. /n /n'    
            
            text += str(self.current_trial['corruption_evidence']) + ' initial evidence - '
            text += str(self.current_trial['num_lawyers']) + ' defense lawyers '
            if self.current_trial['judge_modifier'] < 0:
                text += '- ' + str(abs(self.current_trial['judge_modifier'])) + ' judge bias '
            elif self.current_trial['judge_bias'] > 0:
                text += '+ ' + str(self.current_trial['judge_modifier']) + ' judge bias '
            else:
                text += '+ 0 judge bias '
            text += '= ' + str(self.current_trial['effective_evidence']) + ' evidence rolls /n /n'
        return(text)
    ...
    def middle(self):
        ...
        if prosecutor_corrupt:
            ...
            prosecution.steal_money(trial_utility.get_fabricated_evidence_cost(defense.fabricated_evidence, True), 'trial')
            if flags.prosecution_bribed_judge:
                prosecution.steal_money(trial_utility.get_fabricated_evidence_cost(0), 'trial')

        self.current_trial = {}
        trial_utility.manage_defense(defense.corruption_evidence, prosecutor_corrupt)
        self.current_trial['judge_modifier'] = 0
        self.current_trial['judge_bias'] = prosecution.no_corruption_roll(6) - defense.get_roll_modifier() #D6 with both skill modifiers competing
        ...
        if self.current_trial['trial_rolls']:
            for i in range(0, roll_result_index + 1):
                ...
                constants.notification_manager.display_notification({
                    'message': remaining_rolls_message + text + current_roll_list[1],
                    'notification_type': 'action',
                })
    ...
    def complete(self):
        ...
        if self.roll_result >= self.current_min_success:
            ...
            if confiscated_money > 0:
                text += 'While most of ' + defense.name + '\'s money was spent on the trial or unaccounted for, authorities managed to confiscate ' + str(confiscated_money) + ' money, which has been given to your company as compensation. /n /n'
                text += ' /n /n'
                constants.money_tracker.change(confiscated_money, 'trial_compensation')
            else:
                text += 'Authorities searched ' + defense.name + '\'s properties but were not able to find any stolen money with which to compensate your company. Perhaps it remains hidden, had already been spent, or had never been stolen. /n /n'
            constants.notification_manager.display_notification({
                'message': text,
                'notification_type': 'action',
                'audio': 'guilty'
