            ...
            if self.cell.visible:
                if self.cell.terrain == "water":
                    if coordinates[1] == 0:
                        ...
                        tooltip_message.append(f"    Movement cost: {constants.terrain_movement_cost_dict[self.cell.terrain]} (with steamship)")
                        tooltip_message.append(f"        Otherwise impassable")
                    else:
                        tooltip_message.append("This is a river water tile")
                        tooltip_message.append(f"    Movement cost: {constants.terrain_movement_cost_dict[self.cell.terrain]} (with canoes or steamboat)")
                        tooltip_message.append(f"        Otherwise costs entire turn of movement")
                else:
                    tooltip_message.append(f"This is {utility.generate_article(self.cell.terrain)} {self.cell.terrain} tile")
                    tooltip_message.append(f"    Movement cost: {constants.terrain_movement_cost_dict[self.cell.terrain]}")
                tooltip_message.append(f"    Building cost multiplier: x{constants.terrain_build_cost_multiplier_dict[self.cell.terrain]}")
                attrition_dict = {1: 'light', 2: 'moderate', 3: 'severe'}
                tooltip_message.append(f"    Attrition: {attrition_dict[constants.terrain_attrition_dict[self.cell.terrain]]}")
