...
class event():
    ...
    def remove(self):
        ...
        if self in self.event_manager.event_list:
            self.event_manager.event_list = utility.remove_from_list(self.event_manager.event_list, self)
...
class repeating_event(event):
    ...
    def activate(self):
        ...
        if not self.num_repeats == -1:
            self.num_repeats -= 1
        if self.num_repeats > 0 or self.num_repeats == -1: #if any repeats left or repeats infinitely
            self.event_manager.add_repeating_event(self.function, self.inputs, self.original_activation_time, self.num_repeats)
